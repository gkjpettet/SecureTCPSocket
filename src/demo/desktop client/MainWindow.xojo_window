...
Begin Window MainWindow
   ...
   Begin SecureTCPSocket MySocket
      ...
      TabPanelIndex   =   0
   End
   Begin PushButton ButtonDisconnect
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   "0"
      Cancel          =   False
...
#tag Events ButtonConnect
	#tag Event
		Sub Action()
		  MySocket.Address = AddressField.Text
		  MySocket.Port = Val(PortField.Text)
...
#tag Events MySocket
	#tag Event
		Sub Connected()
		  Output("Connected to " + Me.RemoteAddress)
	...
	#tag Event
		Sub Disconnected()
		  Output("Disconnected from " + Me.RemoteAddress)
	...
	#tag Event
		Sub Error(code As Integer)
		  Output("A socket error occurred: " + Str(code))
...
#tag Events ButtonSend
	#tag Event
		Sub Action()
		  MySocket.SendMessage(Payload.Text, Val(MessageType.Text))
		End Sub
	#tag EndEvent
#tag EndEvents
...
#tag Events ButtonSendFile
	#tag Event
		Sub Action()
		  ...
		  // Send the data.
		  MySocket.SendMessage(mb, Val(MessageType.Text))
		End Sub
	#tag EndEvent
#tag EndEvents
#tag ViewBehavior
